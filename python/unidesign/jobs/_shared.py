...
class JobArtifact:
    ...
    def persist(self, destination: str | Path, filename: str | None = None) -> Path:
        """Copy the artifact to ``destination`` and return the new location.

        Parameters
        ----------
        destination:
            Directory where the artifact should be copied.
        filename:
            Optional override for the target filename. When omitted the source
            filename is preserved.
        """

        target_dir = Path(destination)
        target_dir.mkdir(parents=True, exist_ok=True)
        target_path = target_dir / (filename or self.path.name)
