#!/usr/bin/env python3
"""Repack Rosetta design domains using UniDesign's Python bindings."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Repack design domains stored in ddomain.pickle via UniDesign bindings.",
    )
    parser.add_argument(
        "--structure",
        default="3m0m_minimized_Hs_only.pdb",
        help="Starting WT structure used for every repack run (default: %(default)s).",
    )
    parser.add_argument(
        "--ddomain",
    ...
    parser.add_argument(
        "--output-dir",
        default="wt_domain_repacking_unidesign",
...
def _to_one_letter(code: str) -> str:
    code = code.strip()
    if len(code) == 1:
        return code.upper()
    normalized = code.upper()
    if normalized not in AA_THREE_TO_ONE:
        raise ValueError(f"Unsupported residue code '{code}'")
    return AA_THREE_TO_ONE[normalized]


def _build_design_sites(
    residues: Sequence[Tuple[int, str]],
    chooser: Callable[[Tuple[int, str]], str],
) -> List[SiteSpec]:
    specs: List[SiteSpec] = []
    for position, chain in residues:
        chain_id = chain.strip() or "_"
        allowed = _to_one_letter(chooser((position, chain)))
        specs.append(SiteSpec(chain=chain_id, position=position, allowed=allowed))
    return specs


def _mutate_domain_sequence(
    ...
    trajectories: int,
    quiet: bool,
) -> Tuple[Structure, Dict[str, float] | None]:
    """Use DesignProtein to mutate the provided residues to specific identities."""

    working_structure = base_structure.clone()
    design_sites = _build_design_sites(residues, chooser)
    domain = _restrict_domain_to_structure(working_structure, DesignDomain(design_sites=design_sites))
    ...
    if designer.best_structure is None:
        raise RuntimeError("UniDesign mutation run failed to produce a structure")
    return designer.best_structure, designer.best_sequence_energy
...
def main() -> None:
    ...
    structure_path = _resolve_with_fallback(args.structure)
    ...
    reference_atoms = _heavy_atom_lookup(structure_path)
    template_structure = Structure.from_pdb(structure_path)
    ...
    for site in all_sites:
        ...
        rotlib_file = rotlib if rotlib is not None else DEFAULT_ROTLIB

        ala_structure, _ = _mutate_domain_sequence(
            template_structure,
            residues,
            chooser=lambda _site: "ALA",
            rotlib=rotlib_file,
            trajectories=args.trajectories,
            quiet=args.quiet,
        )
        site_label = f"{site[0]}{site[1]}"
        ala_out = output_dir / f"ala_mutant_{site_label}.pdb"
        ala_structure.handle.write_pdb(str(ala_out))

        wt_structure, wt_energy = _mutate_domain_sequence(
            ala_structure,
            residues,
            chooser=lambda spec, entry_map=entry: entry_map[spec],
            rotlib=rotlib_file,
            trajectories=args.trajectories,
            quiet=args.quiet,
        )

        pdb_out = output_dir / f"wt_repacked_ala_{site_label}.pdb"
        wt_structure.handle.write_pdb(str(pdb_out))
        ...
        print(
            ...
            f"heavy-atom RMSD={rmsd:.4f} Å, output={pdb_out}"
        )
        results.append(
            {
                "site": site_label,
                ...
                "total_energy": None if wt_energy is None else wt_energy.get("total"),
            }
        )

    csv_path = output_dir / "wt_domain_repacking_ala_rmsd.csv"
